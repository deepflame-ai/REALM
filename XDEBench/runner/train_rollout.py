...
def train(args, net, dataset):
    ...
    for step in range(args.num_iterations):
        ...
        for i in range(num_train // batch_size):
            ...
            totalTimeStep = 2 # T - 1
            ts = random.randint(0, T - 1 - totalTimeStep) # 0
            te = ts + totalTimeStep - 1
            input_batch = torch.stack([train_data[idx[b], ts] for b in range(batch_size)]).to(device)
            input_data = net.encoder(input_batch)
            
            for t in range(ts, te):
                if t < te - 1:
                    with torch.no_grad():
                        input_data = net(input_data, coords)
                else: # t = te-1
                    output_pred = net(input_data, coords)
                    
                    output_batch = torch.stack([train_data[idx[b], t+1] for b in range(batch_size)]).to(device)
