...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.__name__ = 'TransolverU'

        self.in_dim = args.out_dim
        ...
        self.preprocess = MLP(self.in_dim + self.space_dim, self.hidden_dim * 2, self.hidden_dim,
        ...
        self.blocks = nn.ModuleList([Transolver_block(num_heads=self.num_heads, hidden_dim=self.hidden_dim, 
                                                      ...
                                                      last_layer=(_ == self.num_layers - 1)) for _ in range(self.num_layers)])
    ...
    def forward(self, x, coords):
        b, c, g = x.shape
        x = torch.cat([x, coords.repeat(b, 1, 1)], dim=1)      # [B, C+2, H, W]
        x = x.permute(0, 2, 1).reshape(b, g, -1)            # [B, H*W, C+2]
        x = self.preprocess(x)
        x = x + self.placeholder[None, None, :]
        for block in self.blocks:
            x = block(x)
        x = x.view(b, g, -1).permute(0, 2, 1)            # [B, out_dim, H, W]
        return x