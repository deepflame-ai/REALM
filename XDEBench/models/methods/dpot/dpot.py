...
class AFNO2D(nn.Module):
    ...
    def forward(self, x, spatial_size=None):
        ...
        # total_modes = H*W // 2 + 1
        kept_modes_h, kept_modes_w = self.modes

        o1_real[:, :kept_modes_h, :kept_modes_w] = self.act(
            torch.einsum('...bi,bio->...bo', x[:, :kept_modes_h, :kept_modes_w].real, self.w1[0]) - \
            torch.einsum('...bi,bio->...bo', x[:, :kept_modes_h, :kept_modes_w].imag, self.w1[1]) + \
            self.b1[0]
        )

        o1_imag[:, :kept_modes_h, :kept_modes_w] = self.act(
            torch.einsum('...bi,bio->...bo', x[:, :kept_modes_h, :kept_modes_w].imag, self.w1[0]) + \
            torch.einsum('...bi,bio->...bo', x[:, :kept_modes_h, :kept_modes_w].real, self.w1[1]) + \
            self.b1[1]
        )

        o2_real[:, :kept_modes_h, :kept_modes_w] = (
            torch.einsum('...bi,bio->...bo', o1_real[:, :kept_modes_h, :kept_modes_w], self.w2[0]) - \
            torch.einsum('...bi,bio->...bo', o1_imag[:, :kept_modes_h, :kept_modes_w], self.w2[1]) + \
            self.b2[0]
        )

        o2_imag[:, :kept_modes_h, :kept_modes_w] = (
            torch.einsum('...bi,bio->...bo', o1_imag[:, :kept_modes_h, :kept_modes_w], self.w2[0]) + \
            torch.einsum('...bi,bio->...bo', o1_real[:, :kept_modes_h, :kept_modes_w], self.w2[1]) + \
            self.b2[1]
        )
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        img_size = (args.shape_list[0], args.shape_list[1])
        patch_size = 8
        ...
        self.patch_embed = PatchEmbed(
            ...
            out_dim=self.embed_dim, act=args.act
        )
        self.latent_size = self.patch_embed.out_size
        self.pos_embed = nn.Parameter(torch.zeros(
            1, self.embed_dim, self.patch_embed.out_size[0], self.patch_embed.out_size[1]
        ))
        ...
        if isinstance(args.modes, int):
            self.modes = (args.modes, args.modes)
        else:
            self.modes = args.modes

        self.blocks = nn.ModuleList([
            Block(mixing_type=self.mixing_type,modes=self.modes,
                width=self.embed_dim, mlp_ratio=self.mlp_ratio, channel_first = True, n_blocks=self.n_blocks,
                double_skip=False, h=h, w=w,act = args.act)
    ...
    def forward(self, x, coords):
        ...
        return x.mean(axis=-2).permute(0, 3, 1, 2)
