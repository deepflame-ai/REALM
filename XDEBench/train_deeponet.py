...
def train_deeponet(args, net, dataset):
    ...
    for step in range(args.num_iterations):
        ...
        for i in range(num_train // batch_size):
            ...
            input_batch = torch.stack([train_data[idx[b], 0] for b in range(batch_size)]).to(device)
            input_time = torch.stack([time[:, random_ts[b]] for b in range(batch_size)])
            output_batch = torch.stack([train_data[idx[b], random_ts[b]] for b in range(batch_size)]).to(device)

            input_batch = net.encoder(input_batch)
            output_gth = net.encoder(output_batch)
            output_pred = net(input_batch, coords, input_time)
