...
def load_pde_dataset(root_dir: str, data_size: List[int], spatial_size: Tuple[int, int] = (128, 128, 128)):
    group_dirs = sorted([
    ...
    for group_idx, group in enumerate(group_dirs):
        ...
        for time_idx, time in enumerate(time_dirs):
            ...
            if var_list is None:        
                ...
                raw_vars = [v for v in raw_vars if (v != 'N2.npy' and v != 'p.npy' and v != 'U.npy')]  
                chem_vars = [v for v in raw_vars if v not in ('Ux.npy', 'Uy.npy', 'Uz.npy', 'T.npy', 'rho.npy')]
                vel_vars = ['Ux.npy', 'Uy.npy', 'Uz.npy']
                temp_var = ['T.npy']
                rho_var = ['rho.npy']
                var_list = chem_vars + temp_var + rho_var + vel_vars
                time_list = time_dirs
...
if __name__ == "__main__":
    ...
    setup_seed(0)
    data_size = [11, 2, 2]
    spatial_size = (80, 80, 200)
    root_dir = '/aisi-nas/baixuan/XDEBench_rawData/poolfire_sample'
    save_path = '/aisi-nas/baixuan/XDEBench_Data/3dPoolfire/data'
    dataset_name = f'poolfire'
    ...
    description = f'3D poolfire dataset'
